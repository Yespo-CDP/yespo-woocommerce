...
class Initialize {
	...
	private function load_classes() {
		$this->classes = \apply_filters( 'yespo_classes_to_execute', $this->classes );
	...
	private function get_classes( string $namespacetofind ) {
		...
		$namespacetofind = 'Yespo\\' . $namespacetofind;

		// In case composer has autoload optimized
		if ( isset( $classmap[ 'Yespo\\Engine\\Initialize' ] ) ) {
		...
		if ( isset( $prefix[ $namespacetofind ] ) ) {
			...
			if ( !WP_DEBUG ) {
				\wp_die( \esc_html__( 'Yespo is on production environment with missing `composer dumpautoload -o` that will improve the performance on autoloading itself.', 'yespo-cdp' ) );
			}
