...
class Yespo_Export_Orders
{
    ...
    public function get_export_orders_count(){
        ...
        return $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM %i WHERE post_type = %s AND post_status != %s AND ID NOT IN ( SELECT post_id FROM {$prefix_postmeta_table} WHERE meta_key = %s AND meta_value = 'true')",$table_posts, 'shop_order', 'wc-checkout-draft', $meta_key));
    }
    ...
    public function get_bulk_export_orders(){
        ...
        // phpcs:ignore WordPress.DB
        return $wpdb->get_results($wpdb->prepare("SELECT ID FROM %i WHERE post_type = %s AND post_status != %s AND ID NOT IN ( SELECT post_id FROM {$prefix_postmeta_table} WHERE meta_key = %s AND meta_value = 'true' ) AND post_date_gmt < %s AND ID > %d ORDER BY ID ASC LIMIT %d",$table_posts, 'shop_order', 'wc-checkout-draft', $meta_key, $period_start, $id_more_then, $number_for_export),OBJECT);
    }
    ...
    private function get_orders_from_database_without_metakey(){
        ...
        // phpcs:ignore WordPress.DB
        return $wpdb->get_results($wpdb->prepare("SELECT * FROM %i WHERE post_type = %s AND post_status != %s AND ID NOT IN ( SELECT post_id FROM {$prefix_postmeta_table} WHERE meta_key = %s AND meta_value = 'true')",
                $table_posts,
                'shop_order',
                'wc-checkout-draft',
                $this->meta_key
    ...
    private function get_orders_from_db($time){
        ...
        return $wpdb->get_results(
            $wpdb->prepare(
                "SELECT * FROM %i WHERE post_type = %s AND post_status != %s AND post_modified_gmt BETWEEN %s AND %s",
                $table_posts,
                'shop_order',
                'wc-checkout-draft',
    ...
    public function get_unsent_orders_since_time($time){
        ...
        // phpcs:ignore WordPress.DB
        return $wpdb->get_results($wpdb->prepare("SELECT p.ID FROM %i p LEFT JOIN {$postmeta} pm ON p.ID = pm.post_id AND pm.meta_key = 'sent_order_to_yespo' WHERE p.post_type = %s AND p.post_status != %s AND p.post_modified_gmt BETWEEN %s AND %s AND pm.post_id IS NULL",
                $table_posts,
                'shop_order',
                'wc-checkout-draft',
