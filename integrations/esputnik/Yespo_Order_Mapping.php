...
class Yespo_Order_Mapping
{
    ...
    private static function order_transformation_to_array($order){
        return [
            ...
            'date' => ($order && !is_bool($order) && $order->get_meta('yespo_order_time'))? self::get_time_order_created($order->get_meta('yespo_order_time')) : (($order && !is_bool($order) && method_exists($order, 'get_date_created') && ($date_created = $order->get_date_created())) ? $date_created->format('Y-m-d\TH:i:s.uP') : null),
